...
extern PRECISION vec2 superpos;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    uv.y *= 95./71.;
    //MOD ALL VALUES BY 1
    //THE ABOVE AND BELOW LINES DUPLICATE THE SCREEN IN A SQUARE ACROSS THE SCREEN
    //uv = mod(uv, 1.);
    //SHIFT THE UV SO THE ORIGIN IS AT THE CENTER
    uv = uv - 0.5;
    //VARIABLES
    float speed = 1.5;
    float abs_thickness = 0.06;
    float brightness = .600;
    float bonus = 0.;
	vec3 color = vec3(0.01, 0., 0.2);
    //FUNCTION
    float ray = abs(sin(u_time*speed+(((uv.x)-(uv.y)) - ((uv.x)+(uv.y))/abs_thickness))/(1.0/brightness)) + bonus;
    float ray2 = abs(sin(PI/2.0+u_time*speed+(((uv.x)-(uv.y)) + ((uv.x)+(uv.y))/abs_thickness))/(1.0/brightness)) + bonus;
    vec3 c1 = vec3(step(ray, 0.4));
    c1 *= vec3(0.390,0.307,0.800);
    
    vec3 c2 = vec3(step(ray2, 0.4));
    c2 *= vec3(0.080,0.269,1.000);
